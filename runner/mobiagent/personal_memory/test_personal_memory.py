...
class PersonalMemoryStoreTests(unittest.TestCase):
    ...
    def test_relation_text_search_uses_fts_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                recent_need_fts_rows = conn.execute(
                    "SELECT relation_id FROM relations_fts WHERE relations_fts MATCH ?",
                    ("近期购物需求",),
                ).fetchall()
                need_clue_fts_rows = conn.execute(
                    "SELECT relation_id FROM relations_fts WHERE relations_fts MATCH ?",
                    ("购物需求线索",),
                ).fetchall()
    ...
    def test_card_linked_event_filter_uses_index_table(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = PersonalMemoryStore(Path(tmp) / "memory.db")
            store.initialize()
            store.upsert_events([
    ...
    def test_v4_migration_rebuilds_relation_fts_for_longer_cjk_terms(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            db_path = Path(tmp) / "memory.db"
            conn = sqlite3.connect(db_path)
            conn.executescript(
                ...
                INSERT INTO schema_meta (key, value)
                VALUES ('personal_memory_schema_version', '4');
                """
            )
            conn.execute(
                """
            ...
            conn.execute(
                ...
                (
                    "rel_shop_need",
                    "behavior_causal",
                    "浏览 建 建材 材 后 后形 形 成 成近 近 期 期购 购 物 物需 需 求 求线 线 索",
                ),
            )
    ...
    def test_relation_linked_event_filter_uses_index_table(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = PersonalMemoryStore(Path(tmp) / "memory.db")
            store.initialize()
            store.upsert_events([
